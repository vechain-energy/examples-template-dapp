...
beforeEach(async function () {
  ...
  contracts.myNFT = await getContractWithProxy('MyNFT')
})

describe('MyNFT', () => {
  describe('Initialization', () => {
    it('sets msg.sender as default admin', async () => {
      const contract = await getContractWithProxy('MyNFT')
  ...
  describe('safeMint(to, uri)', () => {
    it('mints new nft for recipient', async () => {
      await contracts.myNFT.safeMint(users.user1.address, '')
      const balance = await contracts.myNFT.balanceOf(users.user1.address)
      expect(balance).toEqual(BigNumber.from(1))
    })

    it('sets uri correctly', async () => {
      const uri = 'https://vechain.energy'
      const { events } = await (await contracts.myNFT.safeMint(users.user1.address, uri)).wait()
      const { tokenId } = events.find(({ event }) => event === 'Transfer').args

      const tokenUri = await contracts.myNFT.tokenURI(tokenId)
...
async function createToken (...args) {
  const { events } = await (await contracts.myNFT.connect(users.owner).safeMint(...args)).wait()
  const { tokenId } = events.find(({ event }) => event === 'Transfer').args
  return tokenId
}
